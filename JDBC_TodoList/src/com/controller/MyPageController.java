...
//	List<TodoVO> todoList = null;
	...
	// ===오늘 달성률 출력
	public void printTodayAchievementRate(String id) {
		LocalDate today = LocalDate.now();
		int todayAchievementRate = averageAchievementRateByDay(id, today);
	...
	// ===주간 달성률 출력
	public void createWeeksAchievementRate(String id, int year, int month) {
		...
		for (LocalDate monday : mondayList) {
			...
			int weekAchievementRate = averageAchievementRateByWeek(id, monday);
			System.out.println("(달성률 : " + weekAchievementRate + "%)");
			System.out.println();
			for (int i = 0; i < WEEK_DAYS; i++) {
				LocalDate targetDate = monday.plusDays(i);
				System.out.print(targetDate);
				int dayAchievementRate = averageAchievementRateByDay(id, targetDate);
	...
	// ===일별 달성률 산출
	public int averageAchievementRateByDay(String id, LocalDate targetDate) {
		String target = targetDate.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
		List<TodoVO> todoList = myDao.selectByDate(id, target);
	...
	// ===주별 평균 달성률 산출
	private int averageAchievementRateByWeek(String id, LocalDate monday) {
		int sumAchievementRateByDay = 0;
		for (int i = 0; i < WEEK_DAYS; i++) {
			LocalDate targetDate = monday.plusDays(i);
			sumAchievementRateByDay += averageAchievementRateByDay(id, targetDate);
		}
	...
	// ===주간 계획 출력
	public void printWeekTodoList(String id, int year, int month, int menuNum) {
		List<LocalDate> mondayList = CalendarUtil.findByMondayList(year, month);
		...
		for (int i = 0; i < WEEK_DAYS; i++) {
			...
			List<TodoVO> targetTodoList = getTodoListByDay(id, targetDate);
			printTodoListByDay(targetTodoList);
		}
	}

	// ===월간 통계 출력
	public void printMonthStatistics(String id, int year, int month) {

		int achivementRateOfThisMonth = getAchievementRateOfMonth(id, year, month);
		...
		int achievmentRateOfPreviousMonth = getAchievementRateOfMonth(id, yearOfPreviousMonth, previousMonth);
		...
		System.out.println();
		List<String> highRankKeyWordList = getHighRankKeyWordList(id, year, month);
		System.out.print("     ");
	...
	// ===연간 통계 출력
	public void printYearStatistics(String id, int year) {
		List<Integer> statisticsList = getYearStatistics(id, year);
	...
	// ===월별 평균 달성률 산출
	private int getAchievementRateOfMonth(String id, int year, int month) {
		...
		for (int i = 0; i < daysOfMonth; i++) {
			...
			sumAchievementRateOfDay += averageAchievementRateByDay(id, targetDate);
		}
		return sumAchievementRateOfDay / daysOfMonth;
	}

	// ===특정 연도의 월별 평균 달성률 조회
	public List<Integer> getYearStatistics(String id, int year) {
		...
		for (int i = 0; i < 12; i++) {
			...
			statisticsList.add(getAchievementRateOfMonth(id, year, month));
		}
		return statisticsList;
	}

	// ===특정 연도의 월별 평균 달성률 기준 '*' 개수 산출
	...
	// ===일자 검색
	public void searchTodoListByDay(String id, LocalDate targetDate) {
		List<TodoVO> targetTodoList = getTodoListByDay(id, targetDate);
		printTodoListByDay(targetTodoList);
	}

	// ===기간 검색
	public void searchTodoListBetweenDays(String id, LocalDate startDate, LocalDate lastDate) {
		int i = 0;
		...
		while (true) {
			...
			System.out.println(" ( 달성률 : " + averageAchievementRateByDay(id, targetDate) + "% )");
			List<TodoVO> targetTodoList = getTodoListByDay(id, targetDate);
			printTodoListByDay(targetTodoList);
	...
	// ===특정 일자의 TODOList 조회
	private List<TodoVO> getTodoListByDay(String id, LocalDate targetDate) {
		String target = targetDate.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
		return myDao.selectByDate(id, target);
	}
	...
	private void printTodoListByDay(List<TodoVO> targetTodoList) {
		...
		IntStream.range(0, targetTodoList.size()).forEach(index -> {
			System.out.println();
	...
	// ===주요 키워드 조회
	public List<String> getHighRankKeyWordList(String id, int year, int month) {
		List<String> highRankWordList = new ArrayList<>();
		List<String> contentList = getAllContentOfMonth(id, year, month);
	...
	// ===월별 '할일' 모두 조회
	private List<String> getAllContentOfMonth(String id, int year, int month) {
		...
		for (int i = 0; i < daysOfMonth; i++) {
			LocalDate targetDate = firstDay.plusDays(i);
			List<TodoVO> targetDateTodoList = getTodoListByDay(id, targetDate);
